...
def processUser(name,todayMonth):
    ...
    for ulist in user_file:
        ...
        if list[0]==name and mm==todayMonth:
            ...
            monthly = 0.05 / 12
            convert = monthly / 100
            interest = saveCurrent * convert
            i = '%.2f' %interest

            s = Users(list[0],list[1],list[2],list[3],list[4],list[5],list[6],list[7],howmuchMore,i)
            usersList.append(s)
    return usersList
...
def savingHistory(name):
    ...
    for shlist in sh_file:
        ...
        print(mm)
        if list[0] == name and date >= todayDT:
            sh = SavingHistory(list[0],list[1],list[2],list[3],list[4],list[5],list[6],list[7])
            savinghistory.append(sh)
