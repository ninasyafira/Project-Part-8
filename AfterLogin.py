...
def home():
    ...
    savinghistory = []
    savinghistory = Process.savingHistory(session['userid'])

    return render_template('homepage.html', users=usersList,checkMM=months[todayMonth],saving=savings,todayMonth=todayMonth, prevMonth=prevMonth, todayYear=todayYear, prevYear=prevYear,limits=limit,over=over,form=form,user = session['userid'],savingHistory=savinghistory)
...
def selected():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('nov.html',users=usersList,checkMM=months[checkMonth], count=len(usersList),limits=limit,over=over,form=form,savingHistory=savinghistory)
